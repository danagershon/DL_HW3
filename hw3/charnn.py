...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    samples = embedded_text[:N * S].view(N, S, V)
    labels = torch.tensor([char_to_idx[char] for char in text[1:N * S + 1]], device=device).view(N, S)
    # ========================
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    # ====== YOUR CODE: ======
    with torch.no_grad():
        model_input = start_sequence  # first sequence to feed the model
        hidden_states = None  # initial hidden states

        for _ in range(n_chars - len(start_sequence)):
            # convert sequence to one hot
            embedded_input = chars_to_onehot(model_input, char_to_idx).type(torch.float)
            # add batch dimension
            model_input = embedded_input.unsqueeze(0).to(device)
            # feed sequnce to model
            output, hidden_states = model(model_input, hidden_states)
            scores = output.squeeze(0)[-1]
            # calculate predicted next chars probabilities
            ...
            out_text += next_char
            # the next char will be the input of the next iteration
            model_input = next_char
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        num_samples_used = num_batches * self.batch_size

        idx = [
            sample_idx 
            for batch_num in range(num_batches)
            for sample_idx in range(batch_num, num_samples_used, num_batches)
        ]
        # ========================
        return iter(idx)
